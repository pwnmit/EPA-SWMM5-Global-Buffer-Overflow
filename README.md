...
Because strcpy() performs byte serialization mapping sequentially until a null-terminator (\0) is found without validating variable lengths, incoming input sequences exceeding 256 bytes break bounds containment. This directly corrupts adjacent static registers in the .bss memory region, throwing an address violation trap (SIGSEGV) and causing an immediate application abort.
